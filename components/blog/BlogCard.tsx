...
const BlogsGrid = ({ viewAll }: { viewAll: boolean }) => {
    
    const filteredBlogs = viewAll ? blogs : blogs.slice(0, 3);


  return (
    <div className="sm:px-[96px] px-4 py-5 pb-16">
      <h1 className={`text-[50px] font-bold mb-8 ${viewAll ? '':"text-center"}`}>Blogs</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredBlogs.map((blog) => (
          <div key={blog.id} className="border rounded-lg p-4 shadow-md  transition hover:shadow-xl">
            <img
              src={blog.image}
              alt={blog.title}
            ...
            <p className="text-gray-600 mt-2 pb-3">{blog.excerpt}</p>
                {/* <a href={`/blog/${blog.title.toLowerCase().split(" ").join('-')}`} className="text-blue-500 mt-4 inline-block">Read More</a> */}
                <Link type="default" className="mt-4 border border-figma_red text-figma_red px-3 py-[6px]  rounded-md hover:text-white hover:bg-figma_red transition-all duration-500" href={`/blog/${blog.title.toLowerCase().replace(/[^a-zA-Z0-9- ]/g,'').split(" ").join('-')}`}>Read More...</Link>
          </div>
        ))}
      </div>
    </div>
