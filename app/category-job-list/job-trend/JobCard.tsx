...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
  return (
    <div className=" card  py-4   gap-6 hover:bg-[rgb(0,0,0,.1)] duration-500 ">
      ...
      <p className=" font-[800]"> {job.title}</p>
    
      </div>
      <p className="text-[16px] font[600] mt-2 "> {job.company}</p>
     
      <p className="text-[14px] text-gray-500 flex items-center justify-start gap-1 mt-2">
            <FaLocationDot /> {job.location} {/* Include location here */}
          </p>

          <p className="text-[14px] text-gray-500 flex items-center justify-start gap-1 mt-2">
          <BulbOutlined /> {job.skill} {/* Include location here */}
          </p>
      </div>
   ...
   <ClockCircleOutlined />Apply Before : 1 weak from now
                        </p>

                        <p className="text-[14px]  text-gray-500 flex items-center   justify-start gap-1 mt-2 ">
                        <EyeOutlined />View : 407
                                </p>
